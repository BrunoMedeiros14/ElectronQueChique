...
import { cadastrarVendaApi } from '@/api/vendas-api'
import { FormaPagamento } from '@/enums/forma-pagamento'
import { gerarDoublePorValorMonetario } from '@/utils/conversores'
import { ProcurarClienteInput } from '../clientes/procurar-cliente-input'
...
const formSchemaVenda = z.object({
  ...
  valorPago: z.string({ message: 'Campo Obrigatório' }).min(1, { message: 'Campo Obrigatório' }),
  valorTotal: z.number({ message: 'Campo Obrigatório' }).min(1, { message: 'Campo Obrigatório' }),
  troco: z.number(),
...
export function DialogCadastrarVendaBeta({ isOpen }: { isOpen: boolean }) {
  ...
  useEffect(() => {
    if (isOpen) {
      form.setValue('formaPagamento', FormaPagamento.Dinheiro)
      form.setValue('valorTotal', 0)
      form.setValue('valorPago', '')
      form.setValue('troco', 0)
